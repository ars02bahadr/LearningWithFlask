...
user_role_create_model = api.model('UserRoleCreate', {
...
user_role_delete_model = api.model('UserRoleDelete', {
    'id': fields.Integer(required=True, description='User Role ID')
})
...
class UserRoleCreate(Resource):
    ...
    def post(self):
        ...
        return views.create(schemas.UserRoleCreateSchema(**data), db.session)
...
class UserRoleUpdate(Resource):
    ...
    def put(self):  
        """Update a user role"""
        data = request.get_json()
        return views.update(schemas.UserRoleUpdateSchema(**data), db.session)
